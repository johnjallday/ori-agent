...
func TestMigration058CreatesEconomySchema(t *testing.T) {
...
func TestMigration058KeysLedgerEntriesByResourceKindAndReference(t *testing.T) {
	ctx := context.Background()
	db := newEconomyTestDB(t)
...
func TestMigration058KeysPendingHarvestByTaskAndRun(t *testing.T) {
	ctx := context.Background()
	db := newEconomyTestDB(t)
...
func TestMigration058UpgradesV57WithoutChangingExistingRows(t *testing.T) {
	ctx := context.Background()
	dbPath := filepath.Join(t.TempDir(), "v57.db")
	...
	// Rewind to v57 by removing exactly what migration 58 adds. The version
	// delete is `> 57` rather than `= 58` so a future migration does not turn
...
func TestMigration058IsIdempotent(t *testing.T) {
	ctx := context.Background()
	db := newEconomyTestDB(t)
	...
	if err := db.migration058Economy(ctx); err != nil {
...
func TestMigration058TablesAreClassifiedAsAppRecords(t *testing.T) {
