...
func (h *Handler) observeReviewedFileJanitorCreation(request ReviewedTemplateCreationRequest) (ReviewedTemplateCreationResult, error) {
	...
	if folders, ok := h.workspaceTaskStore.(interface {
		...
		ws, err = folders.GetFolderWorkspace(request.Descriptor.WorkspaceID)
	} else {
		ws, err = h.workspaceTaskStore.Get(request.Descriptor.WorkspaceID)
	}
	if err != nil || ws == nil {
	...
	if provenance == nil || provenance.AssistantSetup == nil ||
		provenance.AssistantSetup.RunID != request.Descriptor.RunID ||
		provenance.AssistantSetup.OperationID != request.Descriptor.OperationID ||
		provenance.AssistantSetup.ReviewDigest != request.Descriptor.ReviewDigest ||
	...
	for _, instance := range ws.AgentInstances {
		if instance.RoleID == role.RoleID {
	...
	if role.Action == "create" {
		...
		if !found || profile == nil || profile.AssistantSetup == nil ||
			profile.AssistantSetup.ID != request.Descriptor.ProfileProvenanceID ||
			profile.AssistantSetup.OperationID != request.Descriptor.OperationID ||
