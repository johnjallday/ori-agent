...
func toCandidateDTO(candidate filejanitor.JanitorCandidate, filingRootName string) candidateDTO {
	dto := candidateDTO{
		ID:               candidate.ID,
	...
	if definition, err := filejanitor.LookupCategory(string(candidate.EffectiveCategory())); err == nil {
		if filingRootName == "" {
			filingRootName = filejanitor.DefaultFilingRootName
		}
...
func (h *Handler) GetBatch(w http.ResponseWriter, r *http.Request) {
	...
	var (
		batch      filejanitor.JanitorBatch
		candidates []filejanitor.JanitorCandidate
	)
	if batchID == "latest" {
		var found bool
		...
		if err != nil {
			if errors.Is(err, filejanitor.ErrBatchNotFound) {
...
func filterCandidates(candidates []filejanitor.JanitorCandidate, filter string) []filejanitor.JanitorCandidate {
	if filter == "" {
		return candidates
	}
	out := make([]filejanitor.JanitorCandidate, 0, len(candidates))
...
func matchesCandidateFilter(candidate filejanitor.JanitorCandidate, filter string) bool {
...
func candidateCounts(candidates []filejanitor.JanitorCandidate) map[string]int {
...
func candidateDTOs(candidates []filejanitor.JanitorCandidate, filingRootName string) []candidateDTO {
...
func (h *Handler) ScanNow(w http.ResponseWriter, r *http.Request) {
	...
	batch, created, err := h.service.ScanNow(workspaceID, filejanitor.ScanSourceManual)
	if err != nil {
...
func (h *Handler) UpdateDecisions(w http.ResponseWriter, r *http.Request) {
	...
	updates := make([]filejanitor.DecisionUpdate, 0, len(req.Decisions))
	for _, decision := range req.Decisions {
		normalized := filejanitor.Decision(strings.ToLower(strings.TrimSpace(decision.Decision)))
		switch normalized {
		case filejanitor.DecisionNone, filejanitor.DecisionMove, filejanitor.DecisionSkip:
		...
		updates = append(updates, filejanitor.DecisionUpdate{
	...
	if err != nil {
		switch {
		case errors.Is(err, filejanitor.ErrCandidateNotFound):
			_ = orihttp.RespondNotFound(w, "candidate not found")
		case errors.Is(err, filejanitor.ErrUnknownCategory):
	...
	filingRoot := filejanitor.DefaultFilingRootName
	if err == nil {
...
func (h *Handler) Categories(w http.ResponseWriter, r *http.Request) {
	...
	_ = orihttp.RespondSuccess(w, map[string]any{
		"success":    true,
		"categories": filejanitor.CategoryRegistry,
	})
}
...
func (h *Handler) toPreviewItems(payload []decisionPayload) ([]filejanitor.PreviewRequestItem, error) {
	items := make([]filejanitor.PreviewRequestItem, 0, len(payload))
	for _, entry := range payload {
		operation := filejanitor.Operation(strings.ToLower(strings.TrimSpace(entry.Operation)))
		if operation == "" {
			operation = filejanitor.OperationMove
		}
		if operation != filejanitor.OperationMove && operation != filejanitor.OperationTrash {
		...
		items = append(items, filejanitor.PreviewRequestItem{
...
func (h *Handler) PreviewMoves(w http.ResponseWriter, r *http.Request) {
	...
	preview, err := h.service.PreviewMoves(filejanitor.PreviewRequest{
...
func (h *Handler) ConfirmMoves(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.service.ConfirmMoves(r.Context(), filejanitor.ConfirmRequest{
...
func (h *Handler) Undo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, filejanitor.ErrUndoUnavailable) {
...
func (h *Handler) respondReviewError(w http.ResponseWriter, err error, fallback string) {
	...
	case errors.Is(err, filejanitor.ErrCandidateNotActionable):
	...
	case errors.Is(err, filejanitor.ErrInvalidAction):
...
func (h *Handler) UpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	update := filejanitor.SettingsUpdate{
	...
	if req.ContentMode != nil {
		mode := filejanitor.ContentMode(*req.ContentMode)
...
func (h *Handler) Relink(w http.ResponseWriter, r *http.Request) {
	...
	status, err := h.service.Relink(h.lifecycle(), filejanitor.RelinkRequest{
...
func (h *Handler) lifecycle() filejanitor.WatcherLifecycle {
	if h == nil || h.automation == nil {
		return nil
	}
	if lifecycle, ok := h.automation.(filejanitor.WatcherLifecycle); ok {
		return lifecycle
	}
	return nil
}
