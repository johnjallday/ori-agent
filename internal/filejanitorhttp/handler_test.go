...
func newTestHandler(t *testing.T, owners map[string]string) (*Handler, *fakeStore) {
	...
	service := filejanitor.NewService(filejanitor.NewStore(store), store)
...
func TestGetStatus_ReportsSetupRequiredBeforeSetup(t *testing.T) {
	...
	if readiness["state"] != string(filejanitor.ReadinessSetupRequired) {
...
func TestConfirmSetup_ConfiguresTheWorkspace(t *testing.T) {
	...
	if settings["content_mode"] != string(filejanitor.ContentModeMetadataOnly) {
...
func TestConfirmSetup_RejectsAFileWithAStableCode(t *testing.T) {
	...
	if apiErr["code"] != filejanitor.CodeNotADirectory {
		t.Fatalf("error code = %v, want %q (body: %s)", apiErr["code"], filejanitor.CodeNotADirectory, rec.Body.String())
	}
}
...
func TestGetReadiness_ReturnsComponentChecks(t *testing.T) {
	...
	if len(checks) != len(filejanitor.RequiredComponents) {
...
func TestCategories_ServesTheFixedAllowlist(t *testing.T) {
	...
	if len(categories) != len(filejanitor.CategoryRegistry) {
		t.Fatalf("categories = %d, want %d", len(categories), len(filejanitor.CategoryRegistry))
	}
}
