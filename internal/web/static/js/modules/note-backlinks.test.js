...
const {
  ...
  clearBacklinks,
  announceNoteSaved
} = await import('./note-backlinks.js');
...
test('renderBacklinksInto: paints items and shows section', () => {
  withStubDom(
    doc => {
      const rail = new FakeElement('noteAssistRail');
      rail.hidden = true;
      const section = new FakeElement('noteBacklinksSection');
      section.hidden = true;
      const list = new FakeElement('noteBacklinksList');
      const count = new FakeElement('noteBacklinksCount');
      doc.register(rail);
      doc.register(section);
      doc.register(list);
      doc.register(count);
    },
    () => {
      renderBacklinksInto(document, [
        {
          source_note_id: 'a',
          source_note_name: 'Alpha',
          target_text: 'X',
          context_snippet: 'hi [[X]] there'
        },
