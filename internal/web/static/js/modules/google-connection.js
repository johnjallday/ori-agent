  ...
  class GoogleConnectionManager {
    ...
    bind() {
      ...
      if (this.el.disconnectBtn)
        this.el.disconnectBtn.addEventListener('click', () => this.confirmAccountDisconnect());
      if (this.el.switchBtn)
        this.el.switchBtn.addEventListener('click', () => this.switchAccount());
    }

    async refresh() {
      try {
        const res = await fetch('/api/connections/google/status', {
    ...
    render(conn) {
      ...
      this.hideConfirm();
      const connected =
        conn && conn.subject && conn.state !== 'not_connected' && conn.state !== 'disconnecting';
      if (connected) {
    ...
    renderProducts(grants) {
      ...
      grants.forEach(g => {
        ...
        if (g.product === 'gmail' && !g.enabled) {
          ...
          g.product === 'gmail' &&
          g.health === 'healthy' &&
          !(g.granted_scopes || []).includes(GMAIL_SEND_SCOPE)
        ) {
        ...
        if (
          g.enabled &&
          (g.health === 'reconnect_required' || g.health === 'provider_unavailable')
        ) {
    ...
    async startVaultRepair() {
      try {
        const res = await fetch('/api/connections/google/vault', {
          headers: { Accept: 'application/json' }
        });
        if (!res.ok) return;
        ...
        if (!data || data.action === 'ready') {
          ...
          this.showVaultAction({
            ...
            vaults:
              data && data.vault_id
                ? [{ id: data.vault_id, name: data.vault_name || data.vault_id }]
                : []
          });
          return;
        }
    ...
    renderVaultUnlock(host, data) {
      ...
      const unlock = this.vaultButton('Unlock and continue', 'modern-btn-primary', async () => {
        ...
        if (!res.ok) {
          error.textContent =
            res.message || "Couldn't unlock that vault. Check the password and try again.";
          field.focus();
          return;
        }
      ...
      actions.appendChild(
        this.vaultButton('Cancel', 'modern-btn-secondary', () => this.cancelVaultAction())
      );
    ...
    renderVaultCreate(host) {
      ...
      actions.appendChild(
        this.vaultButton('Create vault and continue', 'modern-btn-primary', async () => {
          ...
          const created =
            (res.data && (res.data.vault || (res.data.data && res.data.data.vault))) || null;
      ...
      actions.appendChild(
        this.vaultButton('Create a new vault', 'modern-btn-secondary', () => {
          this.showVaultAction({
            action: 'create',
            message:
              'Create a vault to store your Google credentials, then Ori will continue enabling Gmail.'
          });
        })
      );
      actions.appendChild(
    ...
    async confirmProductDisconnect(product) {
      ...
      this.renderConfirm({
        ...
        body:
          'This removes ' +
          label +
          ' access from this device. Your Google account and other products stay connected.',
        impacts,
        confirmLabel: 'Disconnect ' + label,
        onConfirm: () =>
          this.doDisconnect(
            '/api/connections/google/product/disconnect?product=' + encodeURIComponent(product)
          )
      });
    }
    ...
    async confirmAccountDisconnect() {
      ...
      this.renderConfirm({
        ...
        body: 'This removes all Google access (Gmail, Calendar, Drive) from this device. Workspaces keep their setup and show "Connection required" until you reconnect.',
        impacts,
    ...
    async switchAccount() {
      ...
      this.renderConfirm({
        ...
        body: 'To switch accounts you must disconnect the current one first. Removed workspace links are NOT restored automatically — even if you reconnect the same account.',
        impacts,
    ...
    renderConfirm(opts) {
      ...
      const affected = (opts.impacts || []).flatMap(p =>
        (p.workspaces || []).map(
          ws => (PRODUCT_LABELS[p.product] || p.product) + ' — ' + (ws.name || ws.id)
        )
      );
      if (affected.length) {
    ...
    async refreshMigratable() {
      ...
      if (!host) return;
      try {
        const res = await fetch('/api/connections/google/migratable', {
          headers: { Accept: 'application/json' }
        });
