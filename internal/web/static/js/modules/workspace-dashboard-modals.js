...
(function() {
    ...
    function openEditScheduleModal(taskId) {
        const sched = state.scheduledTasks.find(s => s.target_task_id === taskId);
        if (!sched || !sched.canvas_node_id) {
            alert('No scheduler found for this task');
            return;
        }

        const nodeWrapper = (state.schedulerNodes || []).find(n => (n.node_id === sched.canvas_node_id) || (n.scheduled_task && n.scheduled_task.canvas_node_id === sched.canvas_node_id));
    ...
    async function submitEditSchedule() {
        ...
        try {
            ...
            const resp = await fetch(`/api/orchestration/workspaces/${encodeURIComponent(state.workspaceId)}/scheduler-nodes/${encodeURIComponent(nodeId)}?studio_id=${encodeURIComponent(state.workspaceId)}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    studio_id: state.workspaceId,
