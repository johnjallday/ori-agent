...
export function createEventStream(workspaceId, handlers) {
  ...
  source.addEventListener('scheduled_task.triggered', event =>
    safe(handlers.onScheduledTaskEvent)(event)
  );
  source.addEventListener('scheduled_task.failed', event =>
    safe(handlers.onScheduledTaskEvent)(event)
  );
  source.addEventListener('scheduled_task.completed', event =>
    safe(handlers.onScheduledTaskEvent)(event)
  );
  source.addEventListener('store_node.write.success', event =>
    safe(handlers.onStoreNodeEvent)(event)
  );
  source.addEventListener('store_node.write.failed', event =>
    safe(handlers.onStoreNodeEvent)(event)
  );
