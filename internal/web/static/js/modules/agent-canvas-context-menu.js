    ...
    switch (action) {
      ...
      case 'remove': {
        ...
        if (confirm(`Remove agent "${displayName}"?`)) {
          ...
          apiDelete(`/api/workspaces/${encodeURIComponent(this.parent.studioId)}/agents/${encodeURIComponent(agentId)}`)
            .then(() => {
  ...
  async bulkDeleteNodes() {
    ...
    if (nodesByType.agent && nodesByType.agent.length > 0) {
      for (const {  node } of nodesByType.agent) {
        try {
          ...
          await apiDelete(`/api/workspaces/${encodeURIComponent(this.parent.studioId)}/agents/${encodeURIComponent(agentId)}`);
    ...
    if (nodesByType.task && nodesByType.task.length > 0) {
      for (const { id } of nodesByType.task) {
        try {
          await apiDelete(`/api/workspaces/${encodeURIComponent(this.parent.studioId)}/tasks/${encodeURIComponent(id)}`);
    ...
    if (nodesByType.store && nodesByType.store.length > 0) {
      for (const { id } of nodesByType.store) {
        try {
          await apiDelete(`/api/workspaces/${encodeURIComponent(this.parent.studioId)}/stores/${encodeURIComponent(id)}`);
    ...
    if (nodesByType.attachment && nodesByType.attachment.length > 0) {
      for (const { id } of nodesByType.attachment) {
        try {
          await apiDelete(`/api/workspaces/${encodeURIComponent(this.parent.studioId)}/attachments/${encodeURIComponent(id)}`);
