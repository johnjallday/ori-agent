...
export class RendererNodes {
  ...
  getWorkflowContainerInfo(task) {
    ...
    const subtaskCount = this.state.tasks.filter(
      t => t.parent_id === task.id || t.parent_task_id === task.id
    ).length;
  ...
  drawWorkflowContainer(task, subtaskCount, index) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(
      deleteBtnX + deleteBtnSize / 2,
      deleteBtnY + deleteBtnSize / 2,
      deleteBtnSize / 2,
      0,
      Math.PI * 2
    );
  ...
  drawTaskCard(task, index) {
    ...
    if (assignedNodeId && toCandidates.length) {
      toAgent =
        toCandidates.find(a => a.nodeId === assignedNodeId || a.id === assignedNodeId) || null;
    }
    ...
    const outputConn = this.state.connections.find(c => c.from === task.id);
    const outputsToCombiner = outputConn
      ? this.parent.getNodeById(outputConn.to)?.type === 'combiner'
      : false;
    ...
    if (task.status === 'in_progress') {
      ...
      if (task.progress && task.progress.current_step) {
        ...
        this.ctx.font = '7px system-ui';
        const stepText =
          task.progress.current_step.substring(0, 20) +
          (task.progress.current_step.length > 20 ? '...' : '');
  ...
  drawAgents() {
    this.state.agents.forEach(agent => {
      ...
      switch (agent.status) {
        case 'active':
          statusColor = '#10b981';
          break; // Green - actively executing
        case 'busy':
          statusColor = '#f59e0b';
          break; // Orange - has queued tasks
        case 'error':
          statusColor = '#ef4444';
          break; // Red - error state
        case 'queued':
          statusColor = '#3b82f6';
          break; // Blue - tasks queued
        default:
          statusColor = '#6b7280'; // Gray - idle
      }
      ...
      const displayName = agent.instanceNumber
        ? `${agent.name} #${agent.instanceNumber}`
        : agent.name;
  ...
  drawAttachments() {
    ...
    this.state.attachments.forEach(attachment => {
      ...
      const badge = attachment.link_url
        ? 'Link'
        : fileMeta?.name
          ? fileMeta.name
      ...
      if (attachment.body) {
        const preview =
          attachment.body.length > 50 ? `${attachment.body.slice(0, 47)}...` : attachment.body;
  ...
  drawStoreNodes() {
    ...
    this.state.storeNodes.forEach(storeNode => {
      ...
      this.ctx.font = '10px system-ui';
      let basedir =
        storeNode.storage_target === 'workspace_folder'
      ...
      this.ctx.beginPath();
      this.ctx.arc(
        inputPortX + inputPortSize / 2,
        inputPortY + inputPortSize / 2,
        inputPortSize / 2,
        0,
        Math.PI * 2
      );
  ...
  drawDirectoryNodes() {
    ...
    this.state.directoryReferences.forEach(directory => {
      ...
      this.ctx.beginPath();
      this.ctx.arc(
        outputPortX + outputPortSize / 2,
        outputPortY + outputPortSize / 2,
        outputPortSize / 2,
        0,
        Math.PI * 2
      );
  ...
  drawWorkspaceFolderNodes() {
    ...
    this.state.workspaceFolders.forEach(folder => {
      ...
      const pathText = this.truncateCanvasText(
        folder.relative_path || folder.path || '',
        cardWidth - 24
      );
      this.ctx.fillStyle = '#64748b';
      this.ctx.font = '10px system-ui';
