...
async function startLegacyPluginSetup(pluginName) {
  try {
    ...
    // Refresh plugins list after setup - call loadPlugins from main plugins module
    if (typeof loadPlugins === 'function') {
      setTimeout(() => {
        loadPlugins();
      }, 1000);
...
window.showLegacyPluginConfigModal = showLegacyPluginConfigModal;
window.prepareCompleteSetupParamsObject = prepareCompleteSetupParamsObject;
window.saveLegacyPluginConfig = saveLegacyPluginConfig;
window.showLegacyPluginSetupModal = showLegacyPluginSetupModal;
