...
// FR 29: the preparation check is gone; the handler offers no such operation.
func TestPreparationCheckIsNotAHandlerOperation(t *testing.T) {
	if _, exists := reflect.TypeOf(&Handler{}).MethodByName("CheckPreparation"); exists {
		t.Fatal("the removed preparation check is still a handler method")
	}
}
