...
func TestLoadOpenAIMetadata_InterfaceAliases(t *testing.T) {
	tmpDir := t.TempDir()
	skillDir := filepath.Join(tmpDir, "skill")
	if err := os.MkdirAll(filepath.Join(skillDir, "agents"), 0o755); err != nil {
		t.Fatalf("mkdir agents: %v", err)
	...
	content := `interface:
  display_name: Interface Friendly
  short_description: Interface short desc
  default_prompt: Use the interface prompt
capabilities:
  planning_profile: true
`
	if err := os.WriteFile(filepath.Join(skillDir, "agents", "openai.yaml"), []byte(content), 0o644); err != nil {
	...
	if meta.DisplayName != "Interface Friendly" {
		t.Fatalf("display name = %q", meta.DisplayName)
	}
	if meta.ShortDescription != "Interface short desc" {
		t.Fatalf("short description = %q", meta.ShortDescription)
	}
