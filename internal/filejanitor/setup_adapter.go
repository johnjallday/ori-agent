...
// LegacySetupAdapterID is the registry key a blueprint manifest may name for this
// domain. It matches projecttemplates.ValidSetupWizardAdapters.
//
// The value is frozen at its pre-rename spelling: it is the literal `adapter`
// string the retired downloads-janitor template.json ships on every one of its
// setup-wizard steps, and canonical/File Janitor blueprints declare
// `file_janitor` instead (see FileJanitorSetupAdapterID). Changing this one
// would strand the retired manifest's steps with no registered adapter,
// breaking setup for every workspace still resolving it by ID.
const LegacySetupAdapterID = "downloads_janitor"

// SetupAdapter answers the Setup Wizard's questions about a File Janitor
...
// alias below (FR-134).
func (a *SetupAdapter) ID() string { return LegacySetupAdapterID }
