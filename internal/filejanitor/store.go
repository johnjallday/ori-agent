...
// LegacyStateDirName is the folder, inside a workspace's own folder, that holds
// every File Janitor record for that workspace. Keeping state in the workspace
...
//
// The value is frozen at its pre-rename spelling: it names a directory that
// already exists on disk for every workspace that ever ran setup. Changing it
// would not migrate that directory — Store would simply start looking in a
// folder that is not there, and an upgraded install would silently lose its
// settings, batches, and history.
const LegacyStateDirName = "downloads-janitor"

// settingsFileName is the settings record inside LegacyStateDirName.
...
var ErrWorkspaceMismatch = errors.New("file janitor state belongs to a different workspace")

// FolderResolver resolves a workspace ID to its folder path on disk.
type FolderResolver = workspace.FolderResolver

// Store is the durable, workspace-scoped home for File Janitor state.
//
...
func (s *Store) StateDir(workspaceID string) (string, error) {
	...
	return filepath.Join(folder, LegacyStateDirName), nil
}
