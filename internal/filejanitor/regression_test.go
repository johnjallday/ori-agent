...
func TestRegression_BindingAliasIsStable(t *testing.T) {
	if LegacyBindingAlias != "downloads_janitor_root" {
		t.Fatalf("the binding alias changed to %q; existing workspaces would keep an orphaned binding", LegacyBindingAlias)
	}
	if LegacyDomainKey != "downloads_janitor" {
		t.Fatalf("the domain key changed to %q; existing watch triggers would stop routing", LegacyDomainKey)
	}
	if LegacyWatchTriggerName == "" || !strings.Contains(LegacyWatchTriggerName, "Downloads Janitor") {
		t.Fatalf("the watcher name must stay recognizable: %q", LegacyWatchTriggerName)
	}
}
