...
// TestLegacyPersistedValuesAreFrozen pins the byte-identical strings an
// upgraded install depends on (PRD FR-24, FR-25).
//
// None of these are cosmetic. Each one is already written, verbatim, into
// something on disk for every workspace that ever ran Downloads Janitor setup:
// a directory name, an MCP binding alias, a trigger's Domain/Name pair, or a
// TemplateProvenance.TemplateID. This package now resolves each of them
// through a `Legacy*`-prefixed identifier, but the identifier is free to
// change — the string after `=` is not. If any of the eight checks below ever
// fails, an upgraded workspace loses its state directory (Store looks in a
// folder that no longer matches), its MCP binding (lookups stop finding it and
// orphan a duplicate), its watch trigger (ensureWatcher installs a second
// one), or its template link (AppliesTo/IsFromTemplate stops recognizing the
// workspace at all).
func TestLegacyPersistedValuesAreFrozen(t *testing.T) {
	cases := []struct {
		name string
		got  string
		want string
	}{
		{"LegacyStateDirName", LegacyStateDirName, "downloads-janitor"},
		{"LegacyDomainKey", LegacyDomainKey, "downloads_janitor"},
		{"LegacySetupAdapterID", LegacySetupAdapterID, "downloads_janitor"},
		{"LegacyBindingAlias", LegacyBindingAlias, "downloads_janitor_root"},
		{"LegacyTemplateID", LegacyTemplateID, "downloads-janitor"},
		{"LegacyWatchTriggerName", LegacyWatchTriggerName, "Downloads Janitor folder watch"},
		// DirectoryRequirementKey never got a Legacy* rename: FR-134 already
		// designed it to accept the canonical file-janitor-root key alongside
		// this one, so nothing forced the identifier to change. Its value is
		// exactly as frozen as the six above — it is what a pre-rename
		// workspace's PendingDirectoryRequirements and TemplateProvenance
		// already carry on disk.
		{"DirectoryRequirementKey", DirectoryRequirementKey, "downloads-root"},
		// The workspacecapability package carries its own copy of this same
		// value (LegacyDirectoryRequirementKeyDownloadsRoot), used to classify
		// a pre-existing directory requirement during the startup migration.
		// The two packages must never drift: a mismatch would make the
		// migration and this package's own directory-matching logic disagree
		// about which key names the legacy requirement.
		{"DirectoryRequirementKey vs workspacecapability's copy", DirectoryRequirementKey, workspacecapability.LegacyDirectoryRequirementKeyDownloadsRoot},
	}
	for _, c := range cases {
		if c.got != c.want {
			t.Errorf("%s = %q, want %q (an upgraded install depends on this exact value)", c.name, c.got, c.want)
		}
	}
}
