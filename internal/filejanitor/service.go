...
// LegacyBindingAlias is the alias of the workspace MCP binding scoped to the
// approved inbox root. It is separate from the workspace's own files binding so
// the janitor root can carry a strictly smaller tool allowlist than the
// workspace folder does.
//
// The value is frozen at its pre-rename spelling: it is already persisted as
// the MCPBinding.Alias on every existing workspace's workspace.json. Every
// lookup that finds, repairs, or removes the Janitor's own binding matches on
// this exact alias; changing it would orphan the existing binding and leave an
// upgraded workspace looking for one that no longer answers to its own name.
const LegacyBindingAlias = "downloads_janitor_root"

// filesystemServerName is the MCP server template both bindings instantiate.
const filesystemServerName = "filesystem"

// JanitorReadTools is the complete set of filesystem MCP tools the Curator
// agent may use against the approved root: listing and metadata only.
//
...
// Service owns File Janitor setup and readiness for every workspace. It is
...
type Status struct {
	...
	// Applies reports whether this workspace is a File Janitor workspace at
...
func (s *Service) AppliesTo(workspaceID string) bool {
	...
	if ws.IsFromTemplate(LegacyTemplateID) {
		return true
	}
...
// LegacyTemplateID is the built-in template a Downloads Janitor workspace was
// created from, before the blueprint was retired in favor of File Janitor. It
// persists in every such workspace's TemplateProvenance, so AppliesTo must keep
// recognizing it by this exact value.
const LegacyTemplateID = "downloads-janitor"
...
func (s *Service) ensureWorkspaceAccess(workspaceID, root string) (string, error) {
	...
	err := s.workspaces.Update(workspaceID, func(ws *workspace.Workspace) error {
		...
		// filesystem binding — linking the janitor folder into that would silently
		// hand the agent mutation tools over the approved root.
...
// roots it covers today. Without this, adding the janitor reference would
// widen that synthesized binding to include the approved root with its full
// tool set.
...
func ensureJanitorBinding(ws *workspace.Workspace, root string) string {
	...
	for i := range ws.MCPBindings {
		if strings.EqualFold(strings.TrimSpace(ws.MCPBindings[i].Alias), LegacyBindingAlias) {
...
func (s *Service) checkBinding(settings JanitorSettings) ComponentCheck {
	...
	for _, binding := range ws.MCPBindings {
		if !strings.EqualFold(strings.TrimSpace(binding.Alias), LegacyBindingAlias) {
			continue
		}
		if !binding.Enabled {
