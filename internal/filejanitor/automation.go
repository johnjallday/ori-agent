...
// LegacyDomainKey is the domain-scan handler key the Janitor registers under.
// It is frozen at its pre-rename spelling because it is already persisted as
// the Domain field on every existing workspace's watch-trigger record;
// changing it would make ensureWatcher stop recognizing that record and
// install a second, duplicate watcher.
const LegacyDomainKey = "downloads_janitor"

// LegacyWatchTriggerName is the name of the trigger the Janitor installs. It is
...
// ensureWatcher matches an existing record on `Name == LegacyWatchTriggerName ||
// Domain == LegacyDomainKey`, and both values are already persisted in the trigger
...
const LegacyWatchTriggerName = "Downloads Janitor folder watch"
...
func (a *Automation) findWatcher(workspaceID string) (*TriggerRecord, error) {
	...
	for i := range records {
		if records[i].Name == LegacyWatchTriggerName || records[i].Domain == LegacyDomainKey {
