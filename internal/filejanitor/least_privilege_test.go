...
// The Curator agent must never hold a tool that can change a file. Approved
...
func TestConfirmSetup_RepairsABindingThatGainedMutationTools(t *testing.T) {
	...
	for i := range ws.MCPBindings {
		if strings.EqualFold(ws.MCPBindings[i].Alias, LegacyBindingAlias) {
...
func TestMCPMover_CallsMoveFileOnTheJanitorBindingOnly(t *testing.T) {
	...
	if materializer.binding.Alias != LegacyBindingAlias {
