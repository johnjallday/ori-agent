...
// testHQ is the workspace every generation in these tests belongs to.
const testHQ = "hq"
...
func startAndFinish(t *testing.T, publisher *recordingActivityPublisher, trigger Trigger) Activity {
	t.Helper()
	got := publisher.all()
	if len(got) != 2 {
	...
	if start.WorkspaceID != testHQ || finish.WorkspaceID != testHQ {
...
func TestActivity_SucceededAndPartialGenerationsReportTheirRevision(t *testing.T) {
	for _, status := range []GenerationStatus{GenerationSucceeded, GenerationPartial} {
		t.Run(string(status), func(t *testing.T) {
			...
			seedConfig(t, store, testHQ)

			rev, err := svc.RequestGenerationNow(context.Background(), testHQ, "local", TriggerScheduled)
			if err != nil {
				t.Fatalf("RequestGenerationNow: %v", err)
			}
			finish := startAndFinish(t, publisher, TriggerScheduled)
...
func TestActivity_FailedGenerationsFinishAsFailed(t *testing.T) {
	...
	t.Run("generator error", func(t *testing.T) {
		...
		svc.SetActivityPublisher(publisher)
		seedConfig(t, store, testHQ)
		rev, _ := svc.RequestGenerationNow(ctx, testHQ, "local", TriggerManual)
		finish := startAndFinish(t, publisher, TriggerManual)
	...
	t.Run("no generator", func(t *testing.T) {
		...
		svc.SetActivityPublisher(publisher)
		seedConfig(t, store, testHQ)
		_, _ = svc.RequestGenerationNow(ctx, testHQ, "local", TriggerScheduled)
		finish := startAndFinish(t, publisher, TriggerScheduled)
	...
	t.Run("store failure", func(t *testing.T) {
		...
		publisher := &recordingActivityPublisher{}
		svc.SetActivityPublisher(publisher)
		seedConfig(t, base, testHQ)
		if _, err := svc.RequestGenerationNow(ctx, testHQ, "local", TriggerScheduled); err == nil {
...
func TestActivity_NilPublisherIsSafe(t *testing.T) {
	...
	svc := NewService(store, &fakeGenerator{result: GenerationResult{Status: GenerationSucceeded}})
	seedConfig(t, store, testHQ)
	if _, err := svc.RequestGenerationNow(context.Background(), testHQ, "local", TriggerScheduled); err != nil {
		t.Fatalf("RequestGenerationNow: %v", err)
	}
}
