...
func registerSessionRoutes(mux *http.ServeMux, s *Server) {
	...
	if s.Handlers.Session != nil {
		...
		// Inert read of an existing project folder chosen with the native picker.
		mux.HandleFunc("POST /api/workspaces/project-connection/review", s.Handlers.Session.ReviewProjectConnection)
		...
		mux.HandleFunc("POST /api/project-templates/{templateID}/setup-quest/preview", s.handleUserSetupQuestPreview)
...
func registerSetupJourneyRoutes(mux *http.ServeMux, s *Server) {
	...
	mux.HandleFunc("GET "+hostQuestRoot+"/runs/{runID}", handler.ScopeHostQuest((*setupjourneyhttp.Handler).GetRun))
...
func registerPersonalHQRoutes(mux *http.ServeMux, s *Server) {
	...
	if s.Handlers.PersonalHQ != nil {
		...
		mux.HandleFunc("GET /api/personal-hq/watchtower", s.Handlers.PersonalHQ.Watchtower)
