...
func renderWorkspaceTasksMarkdownBody(ws *Workspace) string {
	...
	if len(activeRoots) == 0 {
		...
		for _, task := range activeRoots {
			writeTaskMarkdownLine(&sb, ws.FolderSlug, task, children, 0)
		}
	}
	sb.WriteString("\n## Done\n\n")
	if len(doneRoots) == 0 {
...
func writeTaskMarkdownLine(sb *strings.Builder, workspaceSlug string, task Task, children map[string][]Task, depth int) {
	...
	fmt.Fprintf(sb, "%s- [%s] %s%s <!-- %s -->\n", indent, checked, description, agentToken, buildTaskMarkdownMetadata(workspaceSlug, task))
	for _, child := range children[task.ID] {
		writeTaskMarkdownLine(sb, workspaceSlug, child, children, depth+1)
	}
}

func buildTaskMarkdownMetadata(workspaceSlug string, task Task) string {
	...
	parts = append(parts, "url="+escapeTaskMarkdownValue(fmt.Sprintf("/workspaces/%s/task/%s", workspaceSlug, task.ID)))
...
func writeAgentTaskViews(folder string, ws *Workspace) error {
	...
	for agentName, tasks := range byAgent {
		...
		sortTasksForMarkdown(tasks)
		content := renderAgentTaskView(ws.FolderSlug, agentName, tasks)
...
func renderAgentTaskView(workspaceSlug, agentName string, tasks []Task) string {
	var active []Task
	var done []Task
	for _, task := range tasks {
		if task.Status == TaskStatusCompleted {
	...
	sb.WriteString("## Active\n\n")
	writeAgentTaskViewSection(&sb, workspaceSlug, active)
	sb.WriteString("\n## Done\n\n")
	writeAgentTaskViewSection(&sb, workspaceSlug, done)
	return sb.String()
}

func writeAgentTaskViewSection(sb *strings.Builder, workspaceSlug string, tasks []Task) {
	if len(tasks) == 0 {
	...
	for _, task := range tasks {
		...
		taskURL := fmt.Sprintf("/workspaces/%s/task/%s", workspaceSlug, task.ID)
