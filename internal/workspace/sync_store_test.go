...
// TestSyncStore_SavePreservesPinnedReaperScriptsFromStalePrimaryWorkspace mirrors
// TestSyncStore_SavePreservesInstalledCapabilitiesFromStalePrimaryWorkspace for
// PinnedReaperScripts: an unrelated Update starting from a lean SQLite-primary
// projection (which never carries this folder-only field) must not silently
// erase pins that already reached disk.
func TestSyncStore_SavePreservesPinnedReaperScriptsFromStalePrimaryWorkspace(t *testing.T) {
	primary := NewInMemoryStore()
	fileSync, err := NewFileStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	...
	store := NewSyncStore(primary, fileSync)
	ws := newTestWorkspace("ws-pin-sync", "Pin Sync")
	if err := store.Save(ws); err != nil {
		t.Fatal(err)
	}

	// Written directly to fileSync only — bypassing primary entirely — to
	// simulate disk carrying a pin the SQLite-primary projection never saw,
	// exactly like TestSyncStore_SavePreservesInstalledCapabilitiesFromStalePrimaryWorkspace
	// above does for InstalledCapabilities.
	canonicalWorkspace, err := fileSync.Get(ws.ID)
	if err != nil {
		t.Fatal(err)
	}
	canonicalWorkspace.PinnedReaperScripts = []string{"custom:band.lua"}
	canonicalWorkspace.MarkPinnedReaperScriptsExplicit()
	if err := fileSync.Save(canonicalWorkspace); err != nil {
		t.Fatal(err)
	}
	...
	if len(staleWorkspace.PinnedReaperScripts) != 0 {
		t.Fatal("precondition: the stale copy should carry no pins")
	}
	staleWorkspace.Tasks = append(staleWorkspace.Tasks, Task{ID: "after-pin", Status: TaskStatusPending})
	if err := store.Save(staleWorkspace); err != nil {
		t.Fatal(err)
	}
	...
	if len(diskWorkspace.PinnedReaperScripts) != 1 || diskWorkspace.PinnedReaperScripts[0] != "custom:band.lua" {
		t.Fatalf("pin was clobbered by an unrelated task update: %+v", diskWorkspace.PinnedReaperScripts)
	}
	if len(diskWorkspace.Tasks) != 1 || diskWorkspace.Tasks[0].ID != "after-pin" {
		t.Fatalf("task update was not written through: %+v", diskWorkspace.Tasks)
	}
}

// TestSyncStore_SaveWritesThroughDeliberateUnpinToEmpty is the PinnedReaperScripts
// counterpart to TestSyncStore_SaveWritesThroughDeliberateUninstall: unpinning
// the last quick action must reach disk as a real empty state, not be silently
// restored by the stale-write guard.
func TestSyncStore_SaveWritesThroughDeliberateUnpinToEmpty(t *testing.T) {
	primary := NewInMemoryStore()
	fileSync, err := NewFileStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	...
	store := NewSyncStore(primary, fileSync)
	ws := newTestWorkspace("ws-unpin-empty", "Unpin Empty")
	if err := store.Save(ws); err != nil {
		t.Fatal(err)
	}

	pins := NewReaperPinService(store)
	if err := pins.Pin(ws.ID, "custom:band.lua"); err != nil {
		t.Fatal(err)
	}
	if diskWorkspace, err := fileSync.Get(ws.ID); err != nil {
		t.Fatal(err)
	} else if len(diskWorkspace.PinnedReaperScripts) != 1 {
		t.Fatal("precondition: pin should be on disk before unpinning")
	}

	if err := pins.Unpin(ws.ID, "custom:band.lua"); err != nil {
		t.Fatal(err)
	}
	...
	if len(diskWorkspace.PinnedReaperScripts) != 0 {
		t.Fatalf("unpin was undone by the stale-write guard: %+v", diskWorkspace.PinnedReaperScripts)
	}
	...
	if len(primaryWorkspace.PinnedReaperScripts) != 0 {
		t.Fatalf("unpin did not reach the primary store: %+v", primaryWorkspace.PinnedReaperScripts)
	}

	// A truly unrelated Update after the unpin — starting from yet another
	// lean primary projection — must not resurrect the removed pin either.
	if err := store.Update(ws.ID, func(w *Workspace) error {
		w.Tasks = append(w.Tasks, Task{ID: "after-unpin", Status: TaskStatusPending})
		return nil
	}); err != nil {
		t.Fatal(err)
	}
	...
	if len(diskWorkspace.PinnedReaperScripts) != 0 {
		t.Fatalf("unpin was resurrected by a later unrelated update: %+v", diskWorkspace.PinnedReaperScripts)
	}
}
