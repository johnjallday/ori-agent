...
const compactTaskSystemPrompt = "You are completing one task in a shared workspace. Rules:\n" +
	...
	"- The workspace snapshot shows truncated previews. To use a full note, file, session, or directory, call the matching workspace_* tool with its id instead of relying on the preview. Files inside a linked directory are read with workspace_directory_list and workspace_directory_read.\n" +
...
func (h *LLMTaskHandler) buildTaskSystemPrompt(compact bool) string {
	...
	prompt.WriteString("When the workspace has a linked directory, workspace_directory_list and workspace_directory_read browse and read its files as text (PDF, Word, PowerPoint, Excel, and plain text) read-only; take the directory id from workspace_directories first. ")
