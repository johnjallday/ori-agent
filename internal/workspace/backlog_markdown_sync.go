...
type backlogMarkdownSyncState struct {
	...
	// FinalImportAt records the ONE-TIME final import that ends two-way sync
	// (tasks/prd-workspace-ticket-management.md FR-119, FR-120).
	//
	// Before it is set, BACKLOG.md is still a source of truth and file-side
	// edits are imported. After it is set, the file becomes a generated,
	// non-authoritative index: manual edits are never imported and therefore
	// can never mutate a Ticket.
	//
	// The switch is one-way and recorded per workspace, so a workspace that
	// has not been opened since the upgrade still gets its file-side work
	// imported rather than silently discarded.
	FinalImportAt time.Time `json:"final_import_at,omitempty"`
	// LastRenderedHash is the content hash of the body this synchronizer last
	// wrote. A file whose hash no longer matches has been hand-edited since,
	// and must not be overwritten without an explicit choice (FR-122).
	LastRenderedHash string `json:"last_rendered_hash,omitempty"`
}

// backlogMarkdownIsGenerated reports whether this workspace has completed its
// final import and so treats BACKLOG.md as a generated index.
func backlogMarkdownIsGenerated(ws *Workspace) bool {
	return !getBacklogMarkdownSyncState(ws).FinalImportAt.IsZero()
}
...
func renderBacklogMarkdownBody(ws *Workspace) string {
	...
	// The header states the file's actual authority. Once the final import has
	// run this is a GENERATED index (FR-120, FR-121): editing it changes
	// nothing, and saying otherwise would invite a user to lose work.
	if backlogMarkdownIsGenerated(ws) {
		sb.WriteString("> **Generated by Ori — do not edit.** This file is a read-only index of this workspace's Backlog tickets. Edits here are NOT imported and will be replaced the next time it regenerates. Manage tickets in Ori.\n\n")
	} else {
