...
func (h *BackupHandler) HandleExportPluginConfig(w http.ResponseWriter, r *http.Request) {
	...
	if pluginName != "" {
		...
		if err != nil {
			orihttp.InternalError(w, fmt.Sprintf("Failed to export plugin config: %v", err))
			return
		}
		...
		if err != nil {
			orihttp.InternalError(w, fmt.Sprintf("Failed to export all configs: %v", err))
			return
		}
...
func (h *BackupHandler) HandleImportPluginConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.BackupManager.ValidateImportedConfig(configData); err != nil {
		orihttp.BadRequest(w, fmt.Sprintf("Invalid configuration: %v", err))
		return
	}
	...
	if err := h.BackupManager.ImportPluginConfig(configData); err == nil {
		...
		orihttp.InternalError(w, fmt.Sprintf("Failed to import config: %v", err))
		return
	} else {
