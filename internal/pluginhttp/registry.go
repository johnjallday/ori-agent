...
func (h *RegistryHandler) PluginRegistryHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		for _, e := range reg.Plugins {
			if e.Name == req.Name {
				...
				if err != nil {
					orihttp.RespondInternalError(w, fmt.Sprintf("failed to get plugin %s: %v", e.Name, err))
					return
				}
		...
		if !found {
			orihttp.RespondBadRequest(w, "plugin not found in registry")
			return
		}
		...
		if err != nil {
			...
			orihttp.RespondInternalError(w, fmt.Sprintf("failed to load plugin %s", req.Name))
			return
		}
		def := tool.Definition()
	...
	case http.MethodDelete:
		...
		if foundIndex == -1 {
			orihttp.RespondNotFound(w, "plugin not found in local registry (only user uploaded plugins can be deleted)")
			return
		}
...
func (h *RegistryHandler) PluginDownloadHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Download the plugin using the shared downloader (handles caching + platform templates)
	downloadedPath, _, err := h.pluginDownloader.GetPlugin(*pluginEntry)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		if err := json.NewEncoder(w).Encode(map[string]any{
	...
	if abs, err := filepath.Abs(downloadedPath); err == nil {
		downloadedPath = abs
	}

	filename := filepath.Base(downloadedPath)
	if filename == "" || filename == "." || filename == string(os.PathSeparator) {
		filename = fmt.Sprintf("%s%s", pluginEntry.Name, filepath.Ext(downloadedPath))
		if filename == "" {
	...
	// Copy the downloaded file into uploaded_plugins for registry discovery
