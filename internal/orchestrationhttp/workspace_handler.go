...
func (wh *WorkspaceHandler) handleGetWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	if wsID != "" {
		...
		response := map[string]any{
			...
			"workspace_settings_effective_behavior": workspacesettings.BuildEffectiveBehavior(settings),
			"task_markdown_status": workspace.TaskMarkdownStatusForSettings(
				wh.workspaceStore,
				ws.ID,
...
func (wh *WorkspaceHandler) SaveLayoutHandler(w http.ResponseWriter, r *http.Request) {
	...
	var req struct {
		...
		FolderPositions     map[string]workspace.Position        `json:"folder_positions"`
	...
	ws.Layout.FolderPositions = req.FolderPositions
