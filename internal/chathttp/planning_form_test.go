...
func TestMaybeBuildWorkspacePlanningFormResponse_UsesWorkspaceDescriptionNoteDates(t *testing.T) {
	ctx := context.Background()
	db, err := database.Open(ctx, &database.Config{InMemory: true})
	if err != nil {
		t.Fatalf("failed to open test database: %v", err)
	...
	if err := sessionStore.CreateNote(ctx, &session.WorkspaceNote{
		ID:          "workspace-description-portugal",
		WorkspaceID: "workspace-portugal",
		...
		t.Fatalf("failed to create workspace description note: %v", err)
	}
	...
	if !strings.Contains(resp.Form.Summary, "5/11 Lisbon arrival") {
		t.Fatalf("expected summary to reuse workspace description dates, got %q", resp.Form.Summary)
	}
	if !strings.Contains(resp.Form.Summary, `workspace note "Workspace Description"`) {
		t.Fatalf("expected summary to mention workspace description note, got %q", resp.Form.Summary)
	}
	if resp.Form.Questions[0].Required {
		t.Fatalf("expected date field to be optional when workspace description has dates, got %#v", resp.Form.Questions[0])
	}
}
