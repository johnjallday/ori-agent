...
func buildWorkspaceSnapshotPromptForToolCapability(
	...
	if toolCallable {
		lines = append(lines,
			...
			"- workspace_directories: list linked directories and their ids",
			"- workspace_directory_list / workspace_directory_read: browse and read files inside a linked directory as text (PDF, Word, PowerPoint, Excel, and plain text), read-only; present only when the workspace has a linked directory, and they need its id from workspace_directories first",
			"",
			"Management tools:",
