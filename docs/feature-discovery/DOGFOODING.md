...
```bash
./scripts/backlog.sh add "#dogfood <the friction, in one line>" --body "<what you were doing>"
./scripts/backlog.sh                 # what is open, mine, and unplanned
./scripts/backlog.sh view <number>   # the full idea before deciding anything
```
...
**Selecting the day's work stays manual.** `./scripts/backlog.sh` reads and captures;
...
GitHub Issues — the same list / view / add surface `./scripts/backlog.sh` exposes, reachable from a
