...
1. **Build Your Music Production Group** — if Music Project Management is not installed yet, the screen says Music Production Home comes from that separate plugin and offers **Install Music Project Management…**, **Open Plugins**, and **Check Again**. The install shows the release, the minimum reviewed version, the source and everything the plugin registers, then installs and enables it only after you choose **Install**; the screen then moves on by itself. If the plugin is installed but switched off, the screen offers **Enable Music Project Management**; if the two plugins need an update to work together, it offers **Review update**. Once the Home provider is ready, choose **Build Group** to open the shared Create Workspace dialog in its fixed guided-Group context, with **Music Production Home** prefilled. Review the name and build the canonical Music Production Home, or reuse the existing Home unchanged. This creates no agents, projects, schedules, or access grants. The ordinary Group creator instead presents a Group Roster before final creation; its reviewed Manager is scoped to that group's files and notes rather than member project access. Use this screen for the setup-specific Home.
