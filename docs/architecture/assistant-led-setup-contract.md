...
**Status:** Findings-first contract complete. The architecture, recovery protocol, API/state/consent shapes, manual baseline, shared-panel mount, and bounded live-evidence gaps are settled below. No assisted-setup product behavior is implemented by this document.
...
| `POST /api/personal-assistant/setup/file-janitor/runs/{runID}/retry` | `{"if_version":N}` | Explicitly retry only the current verified failed repeat-safe operation after revalidation; it cannot retry a permission, conflict, stale review, or unresolved consequence. |
| `POST /api/personal-assistant/setup/file-janitor/runs/{runID}/manual` | `{"if_version":N}` | Fence automatic continuation, let admitted work settle, and return the exact existing workspace wizard/settings route. Before a run, the GET projection supplies the ordinary blueprint creator route and no mutation is needed. |
...
- `actions`: a server-derived closed list from `accept`, `defer_recommendation`, `choose_target`, `choose_folder`, `prepare_review`, `finish_later`, `resume`, `retry`, `manual`, `open_workspace`, `review_batch`, `pause_monitoring`, `manage_access`, and `history`. Each has only ID, label, enabled, and optional reason/route.
...
The coordinator's retry scheduler is a server-owned background service. It registers a `WorkGate.EnterLifetime` owner when started, acquires a finite permit before dispatching each attempt, and transfers that permit to the goroutine before any HTTP request returns. `Stop` cancels future timers and joins dispatched work. `Server.shutdownBackground` stops this coordinator before File Janitor automation; reset fencing first refuses new finite work, then drain stops both lifetime owners. A normal server shutdown also joins them, even though it does not fence the process-wide gate.
...
| Monitoring approval/registration | `ApproveAutomation` atomically records approval and clears pause; `EnsureWatcher` is a later idempotent trigger-store write. The daily scheduler is one global loop reading settings, not a per-workspace scheduled-job row. | Bind the review to root generation, metadata-only privacy, settings revision, watch events/debounce/exclusions, local time, and timezone. The operation-aware settings write records operation/review IDs before unpausing. A newly created watcher carries the operation ID; a pre-existing watcher is adopted, not claimed. A lost response is reconciled from settings plus the exact trigger. Approval saved with no healthy trigger is a truthful partial success; retry only registration. Scheduler success is fresh global-loop readiness plus matching settings, not a fabricated schedule resource. |
...
File Janitor settings need a monotonic settings revision so review/action compare-and-swap does not depend on timestamp precision. A folder change/revoke continues to issue a new `RootID`; operation-aware folder and monitoring writes also retain their operation ID and resulting revision. `CapabilityResource`/trigger operation markers contain opaque IDs only. The folder path appears only in the File Janitor domain's settings/pending-grant record because that domain must finish or explain the grant; it never enters assistant-setup SQLite, generic logs, URLs, events, or diagnostics.
...
- folder-grant and monitoring writes carry their operation IDs in File Janitor settings and workspace/trigger resource metadata without copying the path into coordinator storage;
