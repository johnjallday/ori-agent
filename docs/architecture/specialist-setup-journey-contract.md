...
### 3.5 Addendum: install split (`setup_quests_v2`)

The five-step `specialist` shape from §3.1 and §3.4 is retired. Installing a
reviewed integration became its own host-generated quest, and plugin and user
quests lost their install step. User guidance is in
[Plugin-owned setup quests](../plugin-setup-quests.md).

| Shape | Ordered step kinds | Who may declare it |
| --- | --- | --- |
| `integration_install` | `integration_install`, `summary` | Generated by the host from the reviewed-integration registry. Never authored. |
...
| `account_link` | `workspace_create`, `account_connect`, `account_link`, `summary` | Ori's embedded host quests (unchanged). |

- **Inference.** The candidate shape is chosen by first kind, then matched
  position by position. A five-step declaration fails with an error naming
  `setup_quests_v2` and the four-step order. `setupJourneyStepOrder` and
  `SetupJourneyRequiredSteps` are deleted; callers use
  `len(SetupJourneyShapeSteps(shape))`.
- **Field rules.** Both `integration_install` and `project_setup` require
  `integration_key`, `expected_blueprint_id` and
  `expected_assistant_program_id`. `integration_install` forbids
  `workspace_launch`. Plugin and user `project_setup` declarations require it,
  with only `group_title` and `group_name`; the runtime title and instructions
  are unknown fields.
- **Plugin contract.** `setup_quests_v2` replaces `setup_quests_v1` in the
  supported host features, and `setup-quest-v2.schema.json` replaces the v1
  schema. A manifest requiring v1 is refused like any unknown feature.
- **Specialist entry.** `Entry.SetupJourney` is replaced by
  `Entry.IntegrationKey`. The compiled compatibility declaration and the legacy
  specialist setup are deleted. Roots created under the old assistant-slug
  identity are no longer reachable.
- **Install quest.** One `install_<integration_key>` declaration per registry
  entry, keyed as a host quest. The catalog lists it only while the plugin is
  not installed; lookup always resolves. Its summary offers the navigation
  actions `continue_integration_setup`, carrying the plugin quest as a handoff,
  and `open_plugins`.
- **Alias.** The accepted assistant's alias resolves to the install quest while
  the plugin is not installed or lists no quest for the integration, and to the
  plugin quest otherwise. The resolution is pinned once per request.
- **Integration precondition.** Every `project_setup` root and child reads the
  `integration_install` reader before its steps. A complete read fills the
  root's integration receipts. Otherwise every step is blocked with the
  integration reason, the lifecycle is `needs_attention`, and the projection
  carries `precondition {reason_code, guidance, integration, install_quest_id}`.
- **Preparation gate removed.** `acknowledge_preparation`, the preparation
  check, its routes and the Home acknowledgement are deleted. The launch view
  has two screens: group, then workspace.
- **Reset instead of migration (§4.5).** An incompatible saved declaration is
  offered **Start over**. `SQLiteStore.DeleteRoot` deletes the root, its
  children and their review, operation and declaration-migration receipts in
  one transaction, refusing a foreign root or one with a busy operation.
  `Service.Restart` requires `DeclarationIncompatible`, deletes the root, then
  reads a fresh one. It is routed as `POST .../restart` on the alias,
  plugin-quest and host-quest roots; user-template scopes return
  `action_unavailable`. Presentation `open` and `dismiss` return an
  incompatible projection without reconciling it. `first_completed_at` history
  is lost on reset.
